...
def calculate_fine(plate):
    violations = {"overspeed": 1000, "no_helmet": 500, "signal_jump": 2000, "unauthorized_vehicle": 1500}
    violation_type = "overspeed"   # Default rule
        ...
        for box in results[0].boxes:
            ...
            if match:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (plate, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), violation_type, fine_amount, "Pending"))
                conn.commit()
...
@app.route('/')
def index():
    return send_from_directory('.', 'index.html')  # Your frontend HTML file
